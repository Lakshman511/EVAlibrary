...
Original file is located at
    https://colab.research.google.com/drive/1s1B5foie_VhhoGlUAg4HTaH1ZqHf_CHC
"""
...
class Train:
  def __init__(self, statsmanager,model, dataloader, optimizer, stats, scheduler=None, L1lambda = 0):
    self.model = model
...
class Test:
  def __init__(self, model, dataloader, stats, statsmanager,scheduler=None):
    self.model = model
    self.dataloader = dataloader
    self.stats = stats
    self.scheduler = scheduler
  ...
  def run(self):
    ...
    with torch.no_grad():
        for data, target in self.dataloader:
            ...
            correct = pred.eq(target.view_as(pred)).sum().item()
            self.statsmanager.append_misclassified_images(data,pred,target)
...
class ModelTrainer:
  def __init__(self, statsmanager,model, optimizer, train_loader, test_loader, statspath,scheduler=None, batch_scheduler=False, L1lambda = 0):
    self.model = model
    self.statsmanager=statsmanager
    self.scheduler = scheduler
    ...
    self.train = Train(statsmanager,model, train_loader, optimizer, self.stats, self.scheduler if self.batch_scheduler else None, L1lambda)
    self.test = Test(model, test_loader, self.stats,statsmanager,self.scheduler)
  ...
  def run(self, epochs=10):
    ...
    for epoch in pbar:
      ...
      self.statsmanager.append_test_accuracy(100*self.stats.test_acc[-1])
      self.statsmanager.append_train_accuracy(100*self.stats.train_acc[-1])
